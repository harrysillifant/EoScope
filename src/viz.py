...
def make_live_animation(history, results_dir='results', fname='training_metrics.mp4', fps=4):
    ...
    def init():
        ax_loss.clear(); ax_sharp.clear(); ax_layer.clear(); ax_ntk.clear(); ax_hess.clear()
        ax_loss.set_title('Train & Test Loss'); ax_loss.set_xlabel('epoch')
        ax_sharp.set_title('Sharpness (max Hessian eigenval)'); ax_sharp.set_xlabel('epoch')
        ax_layer.set_title('Layer spectral norms'); ax_layer.set_xlabel('epoch')
        ax_ntk.set_title('Top-k NTK eigenvalues'); ax_ntk.set_xlabel('epoch')
        ax_hess.set_title('Top-k Hessian eigenvalues'); ax_hess.set_xlabel('epoch')
        return []
    ...
    def update(i):
        ...
        if len(history['ntk_eigs'])>0:
            ...
            for k in range(min(5, ntk_k)):
                vals = [arr[k] if len(arr)>k else np.nan for arr in history['ntk_eigs'][:i+1]]
        ...
        if len(history['hessian_eigs'])>0:
            ...
            for k in range(min(5, h_k)):
                vals = [arr[k] if len(arr)>k else np.nan for arr in history['hessian_eigs'][:i+1]]
    ...
    anim = animation.FuncAnimation(fig, update, frames=epochs, init_func=init, blit=False)
    outpath = os.path.join(results_dir, fname)
    try:
        anim.save(outpath, fps=4, dpi=150)
        print('Saved animation to', outpath)
    except Exception as e:
        print('Could not save mp4 (ffmpeg may be missing). Saving gif instead... Error:', e)
